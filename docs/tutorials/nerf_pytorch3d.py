...
def show_full_render(
    ...
    imageio.imsave(f'results/{time_now}_color.png', (clamp_and_detach(rendered_image) * 255).astype(np.uint8))
    imageio.imsave(f'results/{time_now}_mask.png', (clamp_and_detach(rendered_silhouette) * 255).astype(np.uint8))
    imageio.imsave(f'results/{time_now}_true.png', (clamp_and_detach(target_image) * 255).astype(np.uint8))
...
def segment_image(rgb_image, background_color='white'):
    if background_color == 'white':
        if rgb_image.shape[-1] != 3:
            rgb_image = rgb_image.transpose(1, 2, 0) / 255.
        return 1 - (rgb_image.sum(-1) >= 2.997).astype(np.float)
    else:
        if rgb_image.shape[-1] != 3:
            rgb_image = rgb_image.transpose(1, 2, 0) / 255.
        return 1 - (rgb_image.sum(-1) <= 0.003).astype(np.float)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--h5file', type=str, default='/home/selim/data/nvs/meta-nerf-shapenet-cars-test.h5')
    args = parser.parse_args()

    h5file = h5py.File(args.h5file, 'r')
    model_idx = 1
    target_images = h5file['Images'][model_idx] / 255.
    num_images = 40
    target_images = target_images[:num_images]
    target_images = torch.from_numpy(target_images).float()
    target_images_ = target_images.permute(0,3,1,2)
    target_images = F.interpolate(target_images_, (128, 128,), mode='bilinear').permute(0,2,3,1).numpy()
    ...
    target_silhouettes = torch.from_numpy(target_silhouettes).float().to(device)

    extrinsics = h5file['Extrinsics'][model_idx, :num_images]
    target_cameras = FoVPerspectiveCameras(device=device, R=extrinsics[:, :3, :3], T=extrinsics[:, :3, -1])
    ...
    plt.savefig('results/image_grid.png', format='png')
